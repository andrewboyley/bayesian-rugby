...
export function createGraphSelectionController(graph: GraphLike, renderer: RendererLike) {
  ...
  function updateState(hoveredNode: string | null) {
    ...
    graph.forEachEdge((edge) => {
      ...
      const isSelected =
        hasSelectedPair &&
        ((source === primaryNode && target === secondaryNode) ||
          (source === secondaryNode && target === primaryNode));
      const isPrimaryEdge = hasSelectedPair && (source === primaryNode || target === primaryNode);
      const isActive = hasSelectedPair
        ? isSelected
        : source === (primaryNode ?? hoveredNode) || target === (primaryNode ?? hoveredNode);
    ...
    renderer.setGraphState({
      hasActiveSubgraph: !!activeNodes,
  ...
  function snapshot(): SelectionSnapshot {
    ...
    graph.forEachEdge((edge) => {
      const [source, target] = graph.extremities(edge);
      if (primaryNode && secondaryNode && (source === primaryNode || target === primaryNode))
