...
export function createGraphModel(dataset: GraphDataset): GraphModel {
  ...
  return {
    ...
    nodesByDescendingScore: Uint32Array.from(
      nodes
        .map((_node, index) => index)
        .sort((left, right) => nodes[right].score - nodes[left].score),
    ),
    nodesByDescendingDegree: Uint32Array.from(
      nodes
        .map((_node, index) => index)
        .sort((left, right) => incidentEdges[right].length - incidentEdges[left].length),
    ),
  };
}
