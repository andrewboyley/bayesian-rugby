...
test('OpenSpec graph-projection: reveal operates on deltas while native FA2 remains active', async ({ page }) => {
...
test('OpenSpec graph-projection: reveal tracks rendered state for the full dataset', async ({ page }) => {
	await openProjectionHarness(page);
	...
	await expect.poll(async () => (await snapshot(page)).visibleNodes, { timeout: 30_000 }).toBe(2085);
