  ...
  return page.evaluate(async () => {
    ...
    while (performance.now() < deadline) {
      ...
      if (gl.getQueryParameter(query, gl.QUERY_RESULT_AVAILABLE)) {
        return {
          available: true,
          gpuTimeMs: gl.getQueryParameter(query, gl.QUERY_RESULT) / 1_000_000,
        };
      }
...
async function measureGraph(page: import("@playwright/test").Page, path: string) {
  ...
  url.searchParams.set("test", "projection");
  ...
  await page.evaluate(() => window.rugbyGraphProjectionTest!.reveal());
  await expect(viewer.getByText("nodes 2085 · edges 4906")).toBeVisible({ timeout: 30_000 });
  ...
  return page.evaluate(async () => {
    ...
    const dataset = performance
      ...
      .find((entry) => entry.name.endsWith("/wikipedia.json")) as
    ...
    return {
      ...
      frameIntervals,
      profile:
        (window as Window & { rugbyGraphPerformance?: PerformanceSnapshot })
          .rugbyGraphPerformance ?? null,
    };
  });
}
...
test("enables Sigma diagnostics and reports WebGL GPU timer-query capability", async ({
  page,
}, testInfo) => {
  await measureGraph(page, "/?perf=timers,stats,shaders,picking");
  const profile = await page.evaluate(
    () =>
      (window as Window & { rugbyGraphPerformance?: PerformanceSnapshot }).rugbyGraphPerformance,
  );
